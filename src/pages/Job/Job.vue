<template>
    <h1>Job</h1>
    <!-- Foreach the data from array object -->
    <div v-for="jobs in job" :key="jobs.id">
        <!-- go to JobDetails by id using router-link  -->
       <router-link :to="{name: 'JobDetails' , params: { id: jobs.id }}">
        <h2>{{ jobs.title }}</h2>
       </router-link>
    </div>
</template>
